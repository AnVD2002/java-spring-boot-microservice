package com.project.e_learning_service.service.impl;

import com.project.common_lib_service.dto.CustomUserPrincipal;
import com.project.e_learning_service.client.DocumentServiceClient;
import com.project.e_learning_service.dto.request.*;
import com.project.e_learning_service.dto.response.*;
import com.project.e_learning_service.entity.*;
import com.project.e_learning_service.repository.*;
...
public class ELearningServiceImpl implements ELearningService {
    ...
    @Override
    public LessonDocumentResponse uploadAndAttachDocument(UUID lessonId, UploadLessonDocumentRequest request,
                                                            MultipartFile file, CustomUserPrincipal principal) {
        ensureLessonExists(lessonId);
        RemoteDocumentResponse uploaded = documentServiceClient.upload(file, request, principal);

        LessonDocument document = LessonDocument.builder()
                .lessonId(lessonId)
                .documentId(uploaded.id())
                .documentVersion(uploaded.version() == null ? 1 : uploaded.version())
                .title(request.getTitle())
                .documentType(request.getDocumentType())
                .isRequired(request.getIsRequired() == null || request.getIsRequired())
                .displayOrder(request.getDisplayOrder() == null ? 0 : request.getDisplayOrder())
